...
// Default stats for messages when not provided
...
export default function DashboardHeader({ activeSection, onUploadClick, stats }: DashboardHeaderProps) {
  // Use message stats or default if not provided
  const messageStats = stats?.messages || defaultMessageStats;
  
  // Check if there are new messages to show badge
  const hasNewMessages = messageStats.new > 0;
        ...
        <>
          ...
          {/* <div className="mt-8">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-sm text-gray-500">
                Total: <span className="font-bold text-blue-600">{messageStats.total}</span> messages
              </div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className={`p-4 rounded-xl border ${
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className={`text-2xl font-bold ${messageStats.new > 0 ? 'text-yellow-700' : 'text-gray-700'}`}>
                      {messageStats.new}
                    </div>
                    <div className={`text-sm ${messageStats.new > 0 ? 'text-yellow-600' : 'text-gray-600'} mt-1`}>
                      New
                    </div>
                  </div>
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                    messageStats.new > 0 ? 'bg-yellow-200' : 'bg-gray-200'
                  }`}>
                    <span className={messageStats.new > 0 ? 'text-yellow-700' : 'text-gray-700'}>📨</span>
                  </div>
                </div>
                {messageStats.new > 0 && (
              ...
              <div className="bg-gradient-to-br from-green-50 to-green-100 p-4 rounded-xl border border-green-200">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-2xl font-bold text-green-700">{messageStats.responded}</div>
              ...
              <div className="bg-gradient-to-br from-gray-50 to-gray-100 p-4 rounded-xl border border-gray-200">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-2xl font-bold text-gray-700">{messageStats.archived}</div>
