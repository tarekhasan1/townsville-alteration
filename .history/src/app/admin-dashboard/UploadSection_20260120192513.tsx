...
export default function UploadSection({
  ...
  // Define info for all collections including wedding-gowns
  const collectionInfo: Record<CollectionType, { title: string; desc: string }> = {
    gallery: { 
      title: "Gallery", 
      desc: "General images for gallery section. Recommended: 800x600px" 
    },
    featured: { 
      title: "Featured", 
      desc: "Highlighted images. Recommended: 1200x600px" 
    },
    suits: { 
      title: "Suits", 
      desc: "Suit collection images. Recommended: 600x900px" 
    },
    "wedding-gowns": { 
      title: "Wedding Gowns", 
      desc: "Wedding gown collection. Recommended: 600x900px" 
    },
  };

  // All collection types as array for mapping
        ...
        {allCollections.map((col) => (
          ...
          >
            <div className="text-left">
              <div className="font-medium text-gray-800">{collectionInfo[col].title}</div>
              <div className="text-sm text-gray-500 mt-1">{collectionInfo[col].desc}</div>
              <div className="mt-2 text-xs text-gray-400">Click to select</div>
            </div>
          </button>
        ))}
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-end">
          <button
            ...
            className="px-6 py-3 bg-gradient-to-r from-yellow-400 to-yellow-500 text-white font-medium rounded-lg hover:from-yellow-500 hover:to-yellow-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg"
          >
            {uploading ? (
