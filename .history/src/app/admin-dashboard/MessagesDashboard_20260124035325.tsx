...
export default function MessagesDashboard({ messages: initialMessages, onMessageUpdate }: MessagesDashboardProps) {
  ...
  const loadAllMessages = async () => {
    try {
      ...
      filterMessagesByStatus(result.messages, selectedStatus);
    } catch (error) {
  ...
  const filterMessagesByStatus = (messagesList: ContactMessage[], status: 'all' | 'new' | 'responded' | 'archived') => {
    if (status === 'all') {
      setFilteredMessages(messagesList);
    } else {
      const filtered = messagesList.filter(message => message.status === status);
  ...
  // Update filtered messages when selectedStatus changes
  useEffect(() => {
    filterMessagesByStatus(messages, selectedStatus);
  ...
  const handleMarkAsResponded = async (messageId: string) => {
    ...
    try {
      ...
      await loadAllMessages(); // Reload all messages
      await loadStats();
      onMessageUpdate();
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6">
      ...
      <div className="mb-6">
        <div className="inline-flex rounded-lg border border-gray-200 p-1 bg-gray-50">
          {(['all', 'new', 'responded', 'archived'] as const).map(status => (
            <button
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium capitalize transition-all flex items-center ${
        ...
        <p className="text-sm text-gray-500 mt-2">
          ...
          Filtered by: <span className="font-medium capitalize">{selectedStatus}</span>
        </p>
      </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 bg-gray-50 rounded-xl">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-1">
              No {selectedStatus === 'all' ? '' : selectedStatus} messages found
            </h3>
            <p className="text-gray-600">
              {selectedStatus === 'new' ? 'All messages have been responded to!' :
               selectedStatus === 'responded' ? 'No responded messages yet' :
               selectedStatus === 'archived' ? 'No archived messages yet' :
